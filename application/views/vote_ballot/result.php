...
<div id=content class=container>
    ...
    <?php if ( ! empty($operated)): ?>
    <dl>

        <?php foreach ($operated as $item): ?>
        <dt>
            候选项ID <?php echo $item['option_id'] ?>
            <a href="<?php echo base_url('vote_option/detail?id='.$item['option_id']) ?>">查看</a>
        </dt>
        <dd>操作数量 <?php echo $item['count'] ?></dd>
        <?php endforeach ?>
	...
	<?php if ( !empty($operation) ): ?>
		...
		<li class="col-xs-12 col-sm-6 col-sm-3"><a class="btn btn-default btn-lg" title="继续创建" href="<?php echo base_url($this->class_name.'/create?vote_id='.$vote_id.'&option_id='.$option_id) ?>">再投一票</a></li>
        <?php endif ?>
