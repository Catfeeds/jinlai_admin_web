<?php
	...
	{	
		...
		protected $names_edit_required = array(
			'id','nation', 'province', 'province_index', 'province_abbr', 'province_brief', 'city', 'county',
		);
		...
		public function __construct()
		{
			...
			$this->class_name_cn = '地区'; // 改这里……
			$this->table_name = 'region'; // 和这里……
			$this->id_name = 'region_id'; // 还有这里，OK，这就可以了
			$this->view_root = $this->class_name; // 视图文件所在目录
			$this->media_root = MEDIA_URL. $this->class_name.'/'; // 媒体文件所在目录

			// 设置需要自动在视图文件中生成显示的字段
			$this->data_to_display = array(
				'province_brief' => '省级行政区',
				'city' => '地市级行政区',
                'county' => '区县级行政区',
			);
		} // end __construct
		...
		public function detail()
		{
			...
			if ($result['status'] === 200):
				...
				// 页面信息
                $data['title'] = $this->class_name_cn. '"'.$data['item']['province_brief'].$data['item']['city'].$data['item']['county'].'"';
...
// 			$this->basic->permission_check($role_allowed, $min_level);
			...
			if ($this->form_validation->run() === FALSE):
                ...
                $data_need_no_prepare = array(
                    'province', 'province_index', 'province_abbr', 'city', 'county',
                );
